...
Voice transcription: `ENABLE_VOICE_MESSAGES` (default true), `VOICE_PROVIDER` (`mistral`|`openai`|`local`|`faster-whisper`, default `mistral`), `MISTRAL_API_KEY`, `OPENAI_API_KEY`, `VOICE_TRANSCRIPTION_MODEL`. For `local` provider: `WHISPER_CPP_BINARY_PATH`, `WHISPER_CPP_MODEL_PATH` (requires ffmpeg + whisper.cpp installed). For `faster-whisper` provider (in-process CTranslate2, no subprocess): `FASTER_WHISPER_MODEL` (default `small`; e.g. `tiny`|`base`|`small`|`medium`|`large-v3`|`distil-large-v3`), `FASTER_WHISPER_DEVICE` (`cpu`|`cuda`|`auto`, default `cpu`), `FASTER_WHISPER_COMPUTE_TYPE` (default `int8`; CUDA uses `float16`/`int8_float16`), `FASTER_WHISPER_LANGUAGE` (force a language code, else auto-detect). Model is pulled from HuggingFace on first use into `~/.cache/huggingface/hub` and cached in the process for its lifetime. Provider implementation is in `src/bot/features/voice_handler.py`.
